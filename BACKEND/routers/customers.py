...
@router.get("")
def list_customers(search: Optional[str] = None, db: Session = Depends(get_db), _: User = Depends(require_user)):
    q = db.query(Customer)
    if search:
        like = f"%{search}%"
        q = q.filter(
...
@router.get("/{cust_id}")
def get_customer(cust_id: int, db: Session = Depends(get_db), _: User = Depends(require_user)):
...
@router.post("")
def create_customer(data: CustomerCreate, db: Session = Depends(get_db), _: User = Depends(require_user)):
...
@router.put("/{cust_id}")
def update_customer(cust_id: int, data: CustomerUpdate, db: Session = Depends(get_db), _: User = Depends(require_user)):
    c = db.query(Customer).filter(Customer.id == cust_id).first()
    if not c:
        raise HTTPException(status_code=404, detail="ไม่พบลูกค้า")
...
def delete_customer(cust_id: int, db: Session = Depends(get_db), _: User = Depends(require_user)):
    c = db.query(Customer).filter(Customer.id == cust_id).first()
    if not c:
        raise HTTPException(status_code=404, detail="ไม่พบลูกค้า")
    c.is_active = False
